...
class Underwear extends React.Component {
  ...
  */
  ...
  //  alert(document.querySelector('.fixed-stack').offsetHeight)
  // alert(window.innerHeight)
  // let height = (window.innerHeight - document.querySelector('.fixed-stack').offsetHeight) + 'px'
  //  document.querySelector('.layout-container').style.height =  height
  //  document.querySelector('.layout-adjust').style.height = height
  //  document.querySelector('.page-container').style.height = height
    ...
    return (
      <div className="uw-detail-container" onClick={this.thisHandler}>
        ...
        <div className="adjust-container">
          <NiceShowlayout goodsSelectedId={this.props.params.productId}>
            <UnderweardetailBanner {...this.state.goods}/>
            <ul className="addtion-meta">
              <li>
                <i className="iconfont icon-box" />
                <a href={`${BASE_PAGE_DIR}/box-service`}>担心内衣不合身？打开盒子把试衣间带回家</a>

              </li>
              <li>
                <i className="iconfont icon-zhuanli" />
                <a href={`${BASE_PAGE_DIR}/patent`}>该内衣使用X柔力承托  零负担、无压迫</a>
              </li>
            </ul>

            <ul className="stock">
              <li>库存：<strong>充足</strong></li>
              <li><strong>满76元包邮<i className="iconfont icon-question-mark" /></strong></li>
            </ul>
            <div className="express-tips">默认中通快递发货，如需使用其他快递，请与客服联
系；满76元包邮，不包邮区域：内蒙古，青海，宁夏
，新疆，西藏，海南，甘肃，港澳台以及海外，以上
区域下单时需补邮费差价。</div>
          </NiceShowlayout>
...
Underwear.childContextTypes = {
  productId: React.PropTypes.string.isRequired
}


reactMixin(Underwear.prototype, detailMixins)
