  ...
  render() {
    ...
    for (let i = 1; i < this.props.groupSouce.length; i++ ) {
      ...
      if (this.props.actionModel == ROUTER_SHOPPING_CART_SCAN) {
        boxes.push(<ShoppingCartBoxServiceItem source={itemSource} actionModel={this.props.actionModel}/>)
      }
      // edit model
      if (this.props.actionModel == ROUTER_SHOPPING_CART_EDIT) {
        try{
        ...
        for (; count > 0; count--) {
          boxes.push(<ShoppingCartBoxServiceItem source={update(itemSource, {count: {$set: 1}})} actionModel={this.props.actionModel}/>)
        }
      }
    ...
    return (
      <div>
        <ShoppingCarProductItem source={this.props.groupSouce[0]} actionModel={this.props.actionModel}/>
        {boxes}
      </div>
    )
  }
